...
def api_pagamento_parcial(cid):
    """Registra um pagamento parcial para uma cartela ou lote."""
    d = request.get_json() or {}
    ...
    auditar("PAGAMENTO_PARCIAL", contato_id=cid,
            ...
            detalhes=f"Valor recebido: R$ {valor_num:.2f} | Forma: {forma_pgto} | Recebido por: {recebido_por} | Total pago: R$ {novo_valor_pago:.2f} | Saldo: R$ {novo_saldo:.2f}")

    log(f"Pagamento parcial de R$ {valor_num:.2f} registrado para lote {c['lote']} por {session.get('usuario','?')}", "success")
    return jsonify({
        "ok": True,
        "msg": f"Pagamento de R$ {valor_num:,.2f} registrado!".replace(",", "X").replace(".", ",").replace("X", "."),
...
def api_pagamentos_contato(cid):
    ...
    valor_pago = c.get("valor_pago") or 0
...
@app.route("/api/relatorios/recebimentos")
@requer_login
def api_relatorios_recebimentos():
    """Retorna relatório consolidado de pagamentos parciais."""
    ...
    with get_db() as conn:
        sql = """
            SELECT pp.*, c.lote, c.intervalo, c.nome, c.valor as valor_total,
                   c.status, c.vendedor
            FROM pagamentos_parciais pp
            JOIN contatos c ON c.id = pp.contato_id
            WHERE 1=1
        """
        params = []
        if de:
        ...
        if forma:
            params.append(forma)
            sql += " AND pp.forma_pagamento = ?"
        sql += " ORDER BY pp.id DESC"
        rows = conn.execute(sql, params).fetchall()
        rows = [dict(r) for r in rows]

        # Totalizadores
        total_recebido = sum(r["valor"] for r in rows)
        saldo_aberto = conn.execute(
            "SELECT SUM(saldo_devedor) FROM contatos WHERE LOWER(status)='parcial'"
        ).fetchone()[0] or 0
        total_parciais = conn.execute(
            "SELECT COUNT(*) FROM contatos WHERE LOWER(status)='parcial'"
        ).fetchone()[0] or 0
        total_esperado = conn.execute(
            "SELECT SUM(valor_pago) + SUM(saldo_devedor) FROM contatos WHERE LOWER(status) IN ('parcial','pago') AND valor_pago > 0"
        ).fetchone()[0] or 0
